...
IIC_CODE_IN_RAM void iic_slave_send_data_with_dma_irq(void *arg)
{
    ...
    if (intr_state & DW_IIC_INTR_RD_REQ) {
        flush_dcache_range((unsigned long)iic->data, (iic->size));
...
void csi_iic_uninit(csi_iic_t *iic)
{
    ...
    dw_iic_disable_all_irq(iic_base);
    dw_iic_disable(iic_base);
    disable_irq((uint32_t)iic->dev.irq_num);
    detach_irq((uint32_t)iic->dev.irq_num);
}
...
int32_t csi_iic_master_send(csi_iic_t *iic, uint32_t devaddr, const void *data, uint32_t size, uint32_t timeout, bool stop)
{
    ...
    while (send_count) {
        if (iic_base->IC_STATUS & DW_IIC_TXFIFO_NOT_FULL_STATE) {
            if (--send_count == 0) {
                dw_iic_transmit_data(iic_base, *send_data++ | (stop ? DW_IIC_DATA_STOP : DW_IIC_DATA_RESTART));
            } else {
...
*/
int32_t csi_iic_master_receive(csi_iic_t *iic, uint32_t devaddr, void *data, uint32_t size, uint32_t timeout, bool stop)
{
    CSI_PARAM_CHK(iic, CSI_ERROR);
    CSI_PARAM_CHK(data, CSI_ERROR);
    csi_error_t ret = CSI_OK;
...
csi_error_t csi_iic_master_send_async(csi_iic_t *iic, uint32_t devaddr, const void *data, uint32_t size)
{
    ...
    if (iic->send) {
        ...
        attach_irq((uint32_t)iic->dev.irq_num, &dw_iic_tx_handler, 0, "i2c master send", &iic->dev);
...
csi_error_t csi_iic_master_receive_async(csi_iic_t *iic, uint32_t devaddr, void *data, uint32_t size)
{
    ...
    attach_irq((uint32_t)iic->dev.irq_num, &dw_iic_rx_handler, 0, "iic master rx", &iic->dev);
    enable_irq((uint32_t)iic->dev.irq_num);
...
static csi_error_t iic_slave_send_dma(csi_iic_t *iic, const void *data, uint32_t size)
{
    ...
    attach_irq((uint32_t)iic->dev.irq_num, &iic_slave_send_data_with_dma_irq,  0, "iic slave dma tx",  &iic->dev); ///< if use slave send dma attach will change
    enable_irq((uint32_t)iic->dev.irq_num);
    config.src_inc = DMA_ADDR_INC;
    config.dst_inc = DMA_ADDR_CONSTANT;
    config.src_tw = DMA_DATA_WIDTH_8_BITS;
...
csi_error_t csi_iic_slave_send_async(csi_iic_t *iic, const void *data, uint32_t size)
{
    ...
    if (iic->send) {
        send_func = iic->send;
        attach_irq((uint32_t)iic->dev.irq_num, &iic_slave_send_data_with_dma_irq, 0, "iic slave dma tx", &iic->dev);
        enable_irq((uint32_t)iic->dev.irq_num);
        send_func(iic, data, size);
    } else {
        attach_irq((uint32_t)iic->dev.irq_num, &dw_iic_slave_tx_handler, 0, "iic slave tx", &iic->dev);
        enable_irq((uint32_t)iic->dev.irq_num);
...
csi_error_t csi_iic_slave_receive_async(csi_iic_t *iic, void *data, uint32_t size)
{
    ...
    if (iic->receive) {
        ...
        attach_irq((uint32_t)iic->dev.irq_num, &dw_iic_slave_rx_handler, 0, "iic slave rx", &iic->dev);
        enable_irq((uint32_t)iic->dev.irq_num);
