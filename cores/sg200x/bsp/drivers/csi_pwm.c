...
/**
  ...
  \return      None
*/
csi_error_t csi_pwm_out_config_continuous(csi_pwm_t *pwm,
                                          uint32_t channel,
                                          uint32_t period_us,
                                          ...
                                          csi_pwm_polarity_t polarity)
{
    ...
    if (CSI_OK == ret) {
        cvi_pwm_set_mode(reg_base, (channel & 0x3), true);
...
csi_error_t csi_pwm_out_config_count(csi_pwm_t *pwm,
                               uint32_t channel,
                               uint32_t period_us,
                               uint64_t pcount_us)
{
    ...
    // const uint32_t count_tmp = count_unit * period_us;
    int duty_clk, period_clk;
    uint64_t pcount = pcount_us / period_us;
    ...
    if (CSI_OK == ret) {
        cvi_pwm_set_mode(reg_base, (channel & 0x3), false);

        duty_clk = period_clk / 2;
        cvi_pwm_set_high_period_ch(reg_base, (channel & 0x3), duty_clk);

        cvi_pwm_set_period_ch(reg_base, (channel & 0x3), period_clk);

        cvi_pwm_set_pcount(reg_base, (channel & 0x3), pcount);
