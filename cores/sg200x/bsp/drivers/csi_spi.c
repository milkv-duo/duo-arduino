...
void csi_spi_uninit(csi_spi_t *spi)
{
    ...
    /* unregister irq */
    disable_irq((uint32_t)spi->dev.irq_num);
    detach_irq((uint32_t)spi->dev.irq_num);
}
...
uint32_t csi_spi_baud(csi_spi_t *spi, uint32_t baud)
{
    ...
    spi_base = (dw_spi_regs_t *)HANDLE_REG_BASE(spi);
    dw_spi_config_sclk_clock(spi_base, 100000000, baud);
...
csi_error_t csi_spi_send_async(csi_spi_t *spi, const void *data, uint32_t size)
{
    ...
    if ((ret == CSI_OK) && (spi->callback != NULL)) {
        if (spi->send) {
            ...
            spi->state.writeable = 0U;
            attach_irq((uint32_t)spi->dev.irq_num, &dw_spi_irqhandler, 0, "spi send async", &spi->dev);
            enable_irq((uint32_t)spi->dev.irq_num);
...
static csi_error_t dw_spi_send_dma(csi_spi_t *spi, const void *data, uint32_t size)
{
    ...
    do {
        ...
        config.handshake = 5; //spi_rx_hs_num[spi->dev.idx];

        csi_dma_ch_config(dma_ch, &config);
        dw_spi_config_dma_tx_data_level(spi_base, 8);
        ...
        flush_dcache_range((unsigned long)spi->tx_data, size);
        csi_dma_ch_start(spi->tx_dma, (void *)spi->tx_data, (void *)&(spi_base->DR), size);
    ...
    if ((spi->state.writeable == 0U) || (spi->state.readable == 0U)) {
        ...
        if ((ret == CSI_OK) && (spi->callback != NULL)) {
            if (spi->receive) {
                ...
                spi->state.readable = 0U;
                attach_irq((uint32_t)spi->dev.irq_num, &dw_spi_irqhandler, 0, "spi receive async", &spi->dev);
                enable_irq((uint32_t)spi->dev.irq_num);
...
static csi_error_t dw_spi_receive_dma(csi_spi_t *spi, void *data, uint32_t size)
{
    ...
    do {
        ...
        flush_dcache_range((unsigned long)spi->rx_data, size);
...
csi_error_t csi_spi_send_receive_async(csi_spi_t *spi, const void *data_out, void *data_in, uint32_t size)
{
    ...
    if ((ret == CSI_OK) && (spi->callback != NULL)) {
        if (spi->send_receive) {
            ...
            spi->state.readable = 0U;
            spi->state.writeable = 0U;
            attach_irq((uint32_t)spi->dev.irq_num, &dw_spi_irqhandler, 0, "spi send/receive async", &spi->dev);
            enable_irq((uint32_t)spi->dev.irq_num);
...
static csi_error_t dw_spi_send_receive_dma(csi_spi_t *spi, const void *data_out, void *data_in, uint32_t size)
{
    ...
    do {
        ...
        config.group_len = find_group_len(size, 1U << (uint8_t)config.src_tw);
        config.trans_dir = DMA_MEM2PERH;
        config.handshake = 5; //spi_tx_hs_num[spi->dev.idx];
        csi_dma_ch_config(dma_ch, &config);
        ...
        config.group_len = find_group_len(size, 1U << (uint8_t)config.src_tw);
        config.trans_dir = DMA_PERH2MEM;
        config.handshake = 4; //spi_rx_hs_num[spi->dev.idx];
        csi_dma_ch_config(dma_ch, &config);
...
csi_error_t csi_spi_link_dma(csi_spi_t *spi, csi_dma_ch_t *tx_dma, csi_dma_ch_t *rx_dma)
{
    ...
    if (tx_dma != NULL) {
        tx_dma->parent = spi;
        ret = csi_dma_ch_alloc(tx_dma, 5, 0);
    ...
    if (ret == CSI_OK) {
        if (rx_dma != NULL) {
            rx_dma->parent = spi;
            ret = csi_dma_ch_alloc(rx_dma, 4, 0);
