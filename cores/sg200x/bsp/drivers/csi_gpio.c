...
csi_error_t csi_gpio_init(csi_gpio_t *gpio, uint32_t port_idx)
{
    ...
    dw_gpio_set_port_irq(gpio->dev.reg_base, 0);
    dw_gpio_clr_port_irq(gpio->dev.reg_base, 0U);
...
void csi_gpio_uninit(csi_gpio_t *gpio)
{
    ...
    /* unregister irq */
    disable_irq((uint32_t)gpio->dev.irq_num);
    detach_irq((uint32_t)gpio->dev.irq_num);
...
csi_error_t csi_gpio_attach_callback(csi_gpio_t *gpio, void *callback, void *arg)
{
    ...
    attach_irq((uint32_t)(gpio->dev.irq_num), &dw_gpio_irqhandler, 0, "gpio int", gpio);

    enable_irq((uint32_t)gpio->dev.irq_num);
