...
#define CVI_PWM_MODE_CH_Pos(_ch_)                         (_ch_ + 8)
#define CVI_PWM_MODE_CH_Msk(_ch_)                         (1U << CVI_PWM_MODE_CH_Pos(_ch_))
...
static inline void cvi_pwm_set_mode(unsigned long reg_base, uint32_t ch, bool cont)
{
    if (cont) {
        PWM_POLARITY(reg_base) &= ~CVI_PWM_MODE_CH_Msk(ch);
    } else {
        PWM_POLARITY(reg_base) |= CVI_PWM_MODE_CH_Msk(ch);
    }
}

static inline uint32_t cvi_pwm_get_mode(unsigned long reg_base, uint32_t ch)
{
    return (PWM_POLARITY(reg_base) & CVI_PWM_MODE_CH_Msk(ch));
}

static inline void cvi_pwm_set_pcount(unsigned long reg_base, uint32_t ch, unsigned long pcount)
{
    pcount = pcount > 0xFFFFFF ? 0xFFFFFF : pcount;

    switch (ch) {
    case 0:
        PWM_PCOUNT0(reg_base) = pcount;
        break;
    case 1:
        PWM_PCOUNT1(reg_base) = pcount;
        break;
    case 2:
        PWM_PCOUNT2(reg_base) = pcount;
        break;
    case 3:
        PWM_PCOUNT3(reg_base) = pcount;
        break;
    default:
        break;
    }
