 ...
 */
#define drv_offsetof(type, member)   ((size_t)&(((type *)0)->member))
 ...
 * @param[in]   member  the name of the variable within the struct.
 */
#define drv_container_of(ptr, type, member) \
    ((type *) ((char *) (ptr) - drv_offsetof(type, member)))
...
#define dlist_entry(addr, type, member) \
    ((type *)((long)addr - drv_offsetof(type, member)))
...
static inline void INIT_DRV_DLIST_HEAD(dlist_t *list)
{
    list->next = list;
    list->prev = list;
}
 ...
 */
#define DRV_DLIST_INIT(list)  {&(list), &(list)}
...
#define dlist_for_each_entry(queue, node, type, member) \
    for (node = drv_container_of((queue)->next, type, member); \
         &node->member != (queue); \
         node = drv_container_of(node->member.next, type, member))
...
#define dlist_for_each_entry_safe(queue, n, node, type, member) \
    for (node = drv_container_of((queue)->next, type, member),  \
         ...
         &node->member != (queue);                              \
         node = drv_container_of(n, type, member), n = n ? n->next : NULL)
...
#define list_entry(ptr, type, member) \
        drv_container_of(ptr, type, member)
 ...
 */
#define DRV_DLIST_HEAD_INIT(name) { &(name), &(name) }
 ...
 * @param[in]   name    the list to be initialized.
 */
#define DRV_DLIST_HEAD(name) \
        dlist_t name = DRV_DLIST_HEAD_INIT(name)
...
#define slist_for_each_entry(queue, node, type, member)        \
    for (node = (queue)->next? drv_container_of((queue)->next, type, member) : NULL; \
         node;                                        \
         node = node->member.next ? drv_container_of(node->member.next, type, member) : NULL)
...
#define slist_for_each_entry_safe(queue, tmp, node, type, member) \
    for (node = (queue)->next? drv_container_of((queue)->next, type, member) : NULL, \
         ...
         node;                                           \
         node = tmp ? drv_container_of(tmp, type, member) : NULL, tmp = tmp ? tmp->next : NULL)
 ...
 * @param[in]   name    the list to be initialized.
 */
#define DRV_SLIST_HEAD_INIT(name) {0}
 ...
 * @param[in]   name    the list to be initialized.
 */
#define DRV_SLIST_HEAD(name) \
        slist_t name = DRV_SLIST_HEAD_INIT(name)
...
#define slist_entry(addr, type, member) (                                   \
    addr ? (type *)((long)addr - drv_offsetof(type, member)) : (type *)addr \
)
