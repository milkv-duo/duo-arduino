...
#define DW_IIC_DATA_RESTART_Pos                    (10U)
#define DW_IIC_DATA_RESTART_Msk                    (0x1U << DW_IIC_DATA_RESTART_Pos)
#define DW_IIC_DATA_RESTART                         DW_IIC_DATA_RESTART_Msk
...
static inline void dw_iic_slave_enable_irq(dw_iic_regs_t *iic_base)
{
    iic_base->IC_INTR_MASK = DW_IIC_INTR_STOP_DET | DW_IIC_INTR_RX_FULL | DW_IIC_INTR_RD_REQ;
    iic_base->IC_CLR_INTR;
}
...
{
    pr_printf("[%s]: ===dump start\n\r", __func__);
    pr_printf("IC_CON = %#x\n\r", iic_base->IC_CON);
    pr_printf("IC_TAR = %#x\n\r", iic_base->IC_TAR);
    pr_printf("IC_SAR = %#x\n\r", iic_base->IC_SAR);
    pr_printf("IC_SS_SCL_HCNT = %#x\n\r", iic_base->IC_SS_SCL_HCNT);
    pr_printf("IC_SS_SCL_LCNT = %#x\n\r", iic_base->IC_SS_SCL_LCNT);
    pr_printf("IC_ENABLE = %#x\n\r", iic_base->IC_ENABLE);
    pr_printf("IC_STATUS = %#x\n\r", iic_base->IC_STATUS);
    pr_printf("IC_INTR_MASK = %#x\n\r", iic_base->IC_INTR_MASK);
    pr_printf("IC_INTR_STAT = %#x\n\r", iic_base->IC_INTR_STAT);
    pr_printf("IC_RAW_INTR_STAT = %#x\n\r", iic_base->IC_RAW_INTR_STAT);
    pr_printf("[%s]: ===dump end\n\r", __func__);
}
